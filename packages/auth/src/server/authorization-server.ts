...
export class OAuthAuthorizationServer {
  ...
  private generateAccessToken(claims: Partial<JWTPayload>, upstreamToken?: string): string {
    ...
    const jwtSigningSecret = this.options.jwtSigningSecret || this.options.sessionSecret;
    const jwtEncryptionSecret =
      this.options.jwtEncryptionSecret ||
      Buffer.from(this.options.sessionSecret.padEnd(32, '0').slice(0, 32));
  ...
  getMetadata(): AuthorizationServerMetadata {
    ...
    return {
      ...
      registration_endpoint: this.options.enableDCR ? `${issuer}/oauth/register` : undefined,
  ...
  private async handleAuthCodeGrant(
    ...
    const userId = (pending.userInfo.id ||
      pending.userInfo.sub ||
      pending.userInfo.login ||
      'unknown') as string;
    ...
    const accessToken = this.generateAccessToken(
      ...
      tokens.access_token
    ); // Pass upstream token as second parameter
