...
export class AuthLeanmcp extends AuthProviderBase {
  ...
  async init(config?: {
    ...
    this.orchestrationApiUrl =
      config?.orchestrationApiUrl ||
      process.env.LEANMCP_ORCHESTRATION_API_URL ||
  ...
  async refreshToken(refreshToken: string): Promise<any> {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(
          `Failed to refresh token: ${error.response.data.error?.message || error.response.data.error || 'Unknown error'}`
        );
      }
      throw error;
    }
  ...
  async getUser(token: string): Promise<any> {
    ...
    if (this.apiKey) {
      ...
      try {
        ...
        console.error(
          'Failed to verify user token via Orchestration API:',
  ...
  async getUserSecrets(token: string, projectId: string): Promise<Record<string, string>> {
    ...
    try {
      ...
      console.warn(
        '[LeanMCP] Failed to fetch user secrets:',
