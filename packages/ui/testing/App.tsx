...
import { Sparkles, Command, Fingerprint, Aperture, Grid, Activity, Cpu } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        {showcases.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() =>
                window.scrollTo({ top: 0, behavior: 'smooth' }) || setActiveShowcase(item.id)
              }
              className={`
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${isDark ? 'bg-rose-500 box-shadow-glow' : 'bg-amber-400'}`}
          />
        </button>
      </nav>
