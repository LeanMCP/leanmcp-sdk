...
const RunwaySection = ({
  ...
  <section id={id} className="group relative mb-48 last:mb-0 w-full scroll-mt-32">
    ...
    <div
      className={`flex flex-col ${align === 'center' ? 'items-center text-center' : align === 'left' ? 'items-start text-left' : 'items-end text-right'} relative z-10`}
    >
      {/* Typography */}
      <div className="mb-16 max-w-3xl">
        <div className="flex items-center gap-4 mb-6 opacity-60">
          <div className="h-[1px] w-12 bg-white" />
          <span className="text-xs font-mono uppercase tracking-[0.3em] text-white">
            Component Series
          </span>
          <div className="h-[1px] w-12 bg-white" />
        </div>
        ...
        <div
          className={`h-[1px] w-24 bg-gradient-to-r from-violet-500 to-rose-500 mb-8 ${align === 'center' ? 'mx-auto' : align === 'right' ? 'ml-auto' : ''} group-hover:w-full transition-all duration-1000 ease-out`}
        />
        <p
          className={`text-xl text-zinc-400 font-light leading-relaxed max-w-2xl ${align === 'right' ? 'ml-auto' : ''}`}
        >
          {subtitle}
        </p>
      </div>
      ...
      <div className="flex flex-col items-center gap-12">
        <div className="flex flex-wrap gap-8 items-center justify-center">
          ...
          <ToolButton
            tool="action"
            variant="outline"
            className="rounded-full px-8 py-6 border-white/20 text-zinc-400 hover:text-white hover:border-white transition-colors duration-300"
          >
            Outline Variant
          </ToolButton>
        </div>
        ...
        <div className="w-full max-w-md border-t border-white/5 pt-10 flex flex-col items-center gap-6">
          <span className="text-[10px] uppercase tracking-[0.2em] text-rose-500/80 font-bold border border-rose-500/20 px-3 py-1 rounded-full">
            Destructive Zone
          </span>
          <ToolButton
            tool="delete-item"
            ...
            confirm={{
              ...
              description:
                'You are about to permanently alter the state of this system. This action cannot be rewound.',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 w-full">
        <div className="space-y-6">
          ...
          <p className="text-sm text-zinc-600 font-light">
            Selecting an option immediately triggers a corresponding tool call, updating server
            state without extra clicks.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          />
          <p className="text-sm text-zinc-600 font-light">
            Standard selection mode for local state management or form inputs.
          </p>
        </div>
      </div>
    </RunwaySection>
    ...
    >
      <div className="flex flex-col gap-12 w-full max-w-xl ml-auto">
        ...
        <div className="space-y-6 text-right">
          ...
          />
          <p className="text-sm text-zinc-600 font-light">
            Auto-complete suggestions fetched dynamically from the MCP server as you type.
          </p>
        </div>
      </div>
    </RunwaySection>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 mb-40 w-full" id="data">
      <div className="p-10 rounded-[2.5rem] bg-[#0f0f12]/80 backdrop-blur-md border border-white/5 relative overflow-hidden group hover:border-violet-500/30 transition-colors duration-500">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500">
            Auto-generated schemas that validate and submit directly to tools.
          </p>
        </div>
      ...
      <div className="p-10 rounded-[2.5rem] bg-[#0f0f12]/80 backdrop-blur-md border border-white/5 group hover:border-rose-500/30 transition-colors duration-500 flex flex-col">
        <div className="mb-8">
          ...
          <p className="text-zinc-500">
            High-density data visualization with server-side pagination.
          </p>
        </div>
          ...
          <ToolDataGrid
            ...
            columns={[
              { key: 'name', header: 'Entity Identity' },
              {
                ...
                render: (val) => (
                  <span
                    className={`text-[10px] uppercase tracking-wider px-2 py-1 rounded-sm ${val === 'Online' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                  >
                    {String(val)}
                  </span>
                ),
